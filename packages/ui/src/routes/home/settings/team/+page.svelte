		...
		<div class="flex flex-col">
			{#each filtered as member (member.id)}
				...
				>
					...
					<span class="flex-1 truncate text-xs font-medium">{member.name}</span>
					{#if member.isYou}
	...
	<div class="flex flex-1 flex-col">
		...
		<div class="relative flex items-center gap-4 px-6 py-5">
			...
			<div class="flex flex-col gap-0.5">
				...
				<span class="text-xs text-muted-foreground">{selected.location} · {time}</span>
			</div>
		...
		<Tabs.Root value="about" class="flex flex-col">
			...
			<Tabs.Content value="working-hours" class="mt-0">
				<div class="w-[70%] flex flex-col px-6">
