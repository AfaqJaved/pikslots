<script lang="ts">
	...
	import { getGroupsByClassQueryOptions } from '../api/class-group-assignment/get.groups.by.class.query';
	import { getClassesByBusinessQueryOptions } from '../api/class/get.classes.by.business.query';
	import { updateClassMutationOptions } from '../api/class/update.class.mutation';
	...
	const { classId, onBack }: Props = $props();
	...
	const groupsByClassQuery = createQuery(() => getGroupsByClassQueryOptions(classId));

	const cls = $derived(classesQuery.data?.find((c) => c.id === classId));
	const classGroups = $derived<ClassGroupModel[]>(classGroupsQuery.data ?? []);

	// ── Mutation ─────────────────────────────────────────────────────────────────

	const queryClient = useQueryClient();
	const updateMutation = createMutation(updateClassMutationOptions);
	...
	const { form, errors, enhance } = superForm(
		...
		{
			validators: zod(UpdateClassSchema),
			SPA: true,
			resetForm: false,
			onUpdate({ form }) {
				if (form.valid && classId && businessStore.selectedBusiness?.id) {
	...
	// Pre-fill form when class data loads
	$effect(() => {
		if (cls) {
			$form.title = cls.title;
			$form.description = cls.description;
			$form.durationInMins = cls.durationInMins;
			$form.seats = cls.seats;
			$form.cost = cls.cost / 100;
			$form.isHiddenFromBookingPage = cls.isHiddenFromBookingPage;
		}
	});
...
{:else if !cls}
	<div class="flex flex-1 items-center justify-center">
		<p class="text-sm text-muted-foreground">Class not found.</p>
	</div>
{:else}
	<form use:enhance class="mx-auto flex h-full min-h-0 w-[70%] flex-1 flex-col">
		...
		<div class="flex items-center justify-between border-b px-6 py-3">
			<div class="flex items-center gap-3">
				...
				<span class="text-base font-semibold">Edit class</span>
			</div>
