...
function buildCommand(
  overrides: Partial<UpdateBusinessVisibilityCommand> = {},
): UpdateBusinessVisibilityCommand {
  return {
    id: 'business-1',
...
describe('UpdateBusinessVisibilityUseCaseImpl', () => {
  let useCase: UpdateBusinessVisibilityUseCaseImpl;
  ...
  beforeEach(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      providers: [
        UpdateBusinessVisibilityUseCaseImpl,
        {
    ...
    useCase = moduleRef.get(UpdateBusinessVisibilityUseCaseImpl);
  ...
  // role or ownership check happens anywhere here.
  describe('missing authorization check', () => {
    it('allows an unrelated caller (different business, non-owner role) to change visibility', async () => {
  ...
  describe('successful update', () => {
    it('flips appearInSearchResults from true to false and returns the updated entity', async () => {
      const result = await useCase.execute(
        buildCommand({ id: 'business-1', appearInSearchResults: false }),
      );

      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.appearInSearchResults).toBe(false);
      }
    });

    it('flips appearInSearchResults from false to true', async () => {
      // business-2's fixture has appearInSearchResults: false
      const result = await useCase.execute(
        buildCommand({ id: 'business-2', appearInSearchResults: true }),
      );

      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.appearInSearchResults).toBe(true);
      }
    });
    ...
    it('preserves unrelated fields, like name, slug, and status', async () => {
