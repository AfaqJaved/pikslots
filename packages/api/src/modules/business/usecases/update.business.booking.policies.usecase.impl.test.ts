...
import {
  ...
  IBusinessRepository,
  InfrastructureError,
  UpdateBusinessBookingPoliciesCommand,
...
function buildCommand(
  overrides: Partial<UpdateBusinessBookingPoliciesCommand> = {},
): UpdateBusinessBookingPoliciesCommand {
  return {
    id: 'business-1',
    leadTime: { unit: 'hours', value: 12 },
    scheduleWindow: { unit: 'weeks', value: 4 },
    cancellationPolicy: { unit: 'hours', value: 48 },
...
describe('UpdateBusinessBookingPoliciesUseCaseImpl', () => {
  let useCase: UpdateBusinessBookingPoliciesUseCaseImpl;
  ...
  beforeEach(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      providers: [
        UpdateBusinessBookingPoliciesUseCaseImpl,
        {
    ...
    useCase = moduleRef.get(UpdateBusinessBookingPoliciesUseCaseImpl);
    repository = moduleRef.get(IBusinessRepository);
  });

  describe('missing authorization check', () => {
    it('allows an unrelated caller (different business, non-owner role) to edit the booking policies', async () => {
  ...
  describe('successful update', () => {
    it('updates bookingPolicies and returns the updated entity', async () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.bookingPolicies).toEqual({
          leadTime: command.leadTime,
          scheduleWindow: command.scheduleWindow,
          cancellationPolicy: command.cancellationPolicy,
          bookingPolicyText: command.bookingPolicyText,
          showPolicyOnBookingPage: command.showPolicyOnBookingPage,
        });
      }
    });

    it('allows cancellationPolicy to be cleared to null', async () => {
      // business-1's fixture starts with a non-null cancellationPolicy
      const command = buildCommand({
        id: 'business-1',
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.bookingPolicies.cancellationPolicy).toBeNull();
      }
    });
